...
import ppgAnalysis as ppgAnalysis         # Functions to Analyze the PPG Data
...
if __name__ == "__main__":
    ...
    numChannels = 1               # The Number of Arduino Channels with PPG Signals Read in; My Beta-Test Used 4 Channels
    ...
    calibrateModel = True         # Calibrate the PPG Voltage to Predict the Eye's Angle
    ...
    if readDataFromExcel:
        testDataExcelFile = "../Input Data/PPG Data/All Data/Industry Electrodes/Samuel Solomon 2021-10-08.xlsx" # Path to the Test Data
    ...
    if streamArduinoData:
        arduinoRead = streamData.arduinoRead(eogSerialNum = None, ppgSerialNum = ppgSerialNum, emgSerialNum = None, eegSerialNum = None, handSerialNum = None)
        readData = streamData.ppgArduinoRead(arduinoRead, numTimePoints, moveDataFinger, numChannels, samplingFreq, plotStreamedData, guiApp = None)
        readData.streamPPGData(numDataPoints, calibrateModel = calibrateModel, actionControl = None)
    # Take Data from Excel Sheet
    elif readDataFromExcel:
        ppgProtocol = ppgAnalysis.ppgProtocol(numTimePoints, moveDataFinger, numChannels, samplingFreq, plotStreamedData)
        readData = excelData.readExcel(ppgProtocol)
    ...
    if saveInputData:
        ...
        if verifiedSave.upper() == "Y":
            ...
            saveInputs.saveData(readData.data, None, saveDataFolder, saveExcelName, "Trial 1 - PPG")
        else:
