...
class optimizerMethods:
    ...
    def addOptimizer(self, submodel, model, signalEncoderModel, autoencoderModel, signalMappingModel, sharedEmotionModel, specificEmotionModel):
        ...
        # Set the optimizer.
        constrainedOptimizer = self.setOptimizer(constrainedModelParams, lr=1E-5, weight_decay=1E-10, submodel=submodel, optimizerType=self.userInputParams["optimizerType"])
    ...
    def getLearningRateScheduler(self, submodel, optimizer, constrainedFlag=False):
        ...
        if submodel == "signalEncoder":
            return transformers.get_constant_schedule_with_warmup(optimizer=optimizer, num_warmup_steps=5 if self.useParamsHPC else 5)
        elif submodel == "autoencoder":
